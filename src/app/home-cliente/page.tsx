        ...
        const fetchContracts = async () => {
            if (publicKey) {
                try {
                    ...
                    // Sobrescreve com backend (mas preserva status finais do localStorage)
                    baseContracts.forEach((c: any) => {
                        const local = mergedMap.get(c.id);
                        if (local) {
                            const localStatus = (local.status || "").toLowerCase();
                            const isFinalLocal = localStatus === "cancelado" || localStatus === "alterado" || local.revoked === true;
                            mergedMap.set(c.id, {
                                ...local,
                                ...c,
                                status: isFinalLocal ? localStatus : c.status,
                                revoked: isFinalLocal ? true : c.revoked,
                            });
                        } else {
                    ...
                    // Sobrescreve com blockchain (Verdade absoluta para dados de token/valor)
                    // MAS preserva status finais (cancelado, alterado) que só existem no local/backend
                    onChainContracts.forEach(c => {
                        const existing = mergedMap.get(c.id);
                        if (existing) {
                            const existingStatus = (existing.status || "").toLowerCase();
                            // Statuses irreversíveis: se o backend/local diz que está cancelado ou alterado,
                            // não deixamos o scanner do blockchain sobrescrever (pois o flag on-chain pode não estar setado)
                            const isFinalStatus = existingStatus === "cancelado" || existingStatus === "alterado" || existing.revoked === true;

                            mergedMap.set(c.id, {
                                ...existing,
                                ...c,
                                reclaimed: true,
                                // Preserva status finais do local/backend
                                status: isFinalStatus ? existingStatus : c.status,
                                revoked: isFinalStatus ? true : c.revoked,
                            });
                        } else {
                            mergedMap.set(c.id, c);
                        }
            ...
            try {
                ...
                const updated = allContracts.map((contract: any) => {
                    ...
                    if (startDate && c.vestingDuration) {
                        ...
                        const currentRawStatus = (c.status || "").toLowerCase();

                        // Se foi cancelado (on-chain ou local) ou alterado manualmente, preservamos o status para a UI
                        if (currentRawStatus === "cancelado" || c.revoked === true || currentRawStatus === "alterado") {
                            newStatus = currentRawStatus;
                        } else if (progress >= 1) {
